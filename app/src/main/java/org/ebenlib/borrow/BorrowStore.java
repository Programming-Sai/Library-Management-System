...
public class BorrowStore {
    ...
    public boolean updateStatus(int id, Status to) {
        for (BorrowRecord r : cache) {
            if (r.getId() == id) {
                r.status = to; // we allow internal status change
                return true;
            }
        }
        return false;
    ...
    public PriorityQueue<BorrowRecord> getOverdueQueue(int overdueDays) {
        ...
        PriorityQueue<BorrowRecord> pq = new PriorityQueue<>(Comparator.comparing(BorrowRecord::getDate));
        for (BorrowRecord r : cache) {
            if (r.getStatus() == Status.APPROVED && r.getDate().isBefore(threshold)) {
                pq.add(r);
            }
        }
        return pq;
