...
import org.ebenlib.ds.EbenLibHashSet;
...
public class InteractiveMenus {
    ...
    public static final EbenLibHashSet<String> NAVIGATION_COMMANDS = EbenLibHashSet.of(
        "User", "Book", "Borrow", "Profile", "System", "Report"
    );


    public static EbenLibMap<String, Runnable> getMainMenu(String role, String username) {
        EbenLibMap<String, Runnable> menu = new EbenLibMap<>(true, null);
    ...
    public static EbenLibMap<String, Runnable> getUnauthenticatedMenu() {
        EbenLibMap<String, Runnable> menu = new EbenLibMap<>(true, null);
    ...
    public static EbenLibMap<String, Runnable> getUserMenu(String role) {
        EbenLibMap<String, Runnable> menu = new EbenLibMap<>(true, null);
        if (role.equalsIgnoreCase("Librarian")) {
    ...
    public static EbenLibMap<String, Runnable> getBookMenu(String role) {
        EbenLibMap<String, Runnable> menu = new EbenLibMap<>(true, null);
        if (role.equalsIgnoreCase("Librarian")) {
    ...
    public static EbenLibMap<String, Runnable> getBorrowMenu(String role) {
        EbenLibMap<String, Runnable> menu = new EbenLibMap<>(true, null);
        if (role.equalsIgnoreCase("Librarian")) {
            menu.put("List All Pending Borrow Requests",    () -> runWithPause(() -> BorrowHandler.handleList(EbenLibMap.of("status","PENDING"))));
            menu.put("List All Borrow Operations",    () -> runWithPause(() -> BorrowHandler.handleHistory(EbenLibMap.empty(), true)));
        ...
        menu.put("My Borrow History",() -> runWithPause( () -> BorrowHandler.handleHistory(EbenLibMap.empty(), false)));
        menu.put("Back",           () -> {});
        menu.put("Exit",           () -> System.exit(0));
        return menu;
    }

    public static EbenLibMap<String, Runnable> getProfileMenu(String role) {
        EbenLibMap<String, Runnable> menu = new EbenLibMap<>(true, null);
        menu.put("View Profile", () -> runWithPause(() -> {
            var current = AuthHandler.getCurrentUser();
            if (current != null) ProfileHandler.handle(new String[]{"profile", "view"}, EbenLibMap.empty());
        }));

        menu.put("Update Profile", () -> runWithPause(() -> {
            var current = AuthHandler.getCurrentUser();
            if (current != null) ProfileHandler.handle(new String[]{"profile", "update"}, EbenLibMap.empty());
        }));

        menu.put("Change Password", () -> runWithPause(() -> {
            var current = AuthHandler.getCurrentUser();
            if (current != null) ProfileHandler.handle(new String[]{"profile", "password"}, EbenLibMap.empty());
        }));
        menu.put("Back",            () -> {});
        menu.put("Exit",            () -> System.exit(0));
        return menu;
    ...
    public static EbenLibMap<String, Runnable> getSystemMenu(String role) {
        EbenLibMap<String, Runnable> menu = new EbenLibMap<>(true, null);
        if (role.equalsIgnoreCase("Librarian")) {
    ...
    public static EbenLibMap<String, Runnable> getReportMenu(String role) {
        EbenLibMap<String,Runnable> m = new EbenLibMap<>(true, null);
